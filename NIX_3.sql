...
--1
	SELECT TOP 1 MAX([UnitPrice]) as [price?], [ProductName], [CategoryID]  
	from [dbo].[Products]
	WHERE [CategoryID] = 1 
	GROUP BY [ProductName], [UnitPrice], [CategoryID] 
	ORDER BY[UnitPrice] DESC 
--2
	SELECT [ShipCity] 
	FROM [Orders] 
	where DATEDIFF(day, [OrderDate], [RequiredDate]) > 10
	group by ShipCity
--4
	SELECT count(distinct CustomerID)[count] 
	from Orders as custORDER 
	where custORDER.EmployeeID = 
...
--5
	select [EmployeeID], [OrderDate], count(ShipCity) cnt, [ShipCity] 
	from Orders WHERE [EmployeeID] = 1 AND [ShipCountry] = 'France'
	Group by [EmployeeID], [OrderDate], [ShipCity]
--6
	...
	join
			...
			Having Count(OrderID) > 2) as cityORDER
